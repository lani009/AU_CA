...
/* 전역 변수 정의 */
int32_t process_length = 0; // 입력받은 프로세스 개수
...
Process *get_process_input()
{
    ...
    int32_t process_size = 5; // 받을 수 있는 프로세스 개수

    Process *process_list = malloc(sizeof(Process) * process_size); // 프로세스 배열 메모리 할당
    int32_t process_input = 0;                                      // 입력받은 프로세스 개수
...
void sim_fcfs()
{
    ...
    write_log("****FCFS Scheduling Simulation****\n");
    for (int32_t current_time = 0; current_time < TIMEMAX; current_time++)
    {
        Process *in_process = get_process_in_time(current_time); // time (ms)에 arrival한 프로세스를 받아온다.
        if (!is_null_process(in_process))
        {
            // 새로 들어온 프로세스가 있을 경우
            write_log("%2zu(ms): p%-2zu process arrival\n", current_time, in_process->pid);
        ...
        if (is_null_process(running_process))
        {
            ...
            {
                // 큐에도 프로세스가 없을 경우
                write_log("%2zu(ms): CPU 노는 중\n", current_time);
                if (is_done(current_time))
                {
                    write_log("FCFS Scheduling Simulation Exit\n\n");
    ...
    int32_t waiting_time_sum = 0;

    write_log("FCFS waiting time\n");
    for (int32_t i = 0; i < process_length; i++)
    {
        write_log("p%-2zu  %2zu(ms)\n", process_vec[i].pid,
...
void sim_rr()
{
    write_log("****RR Scheduling Simulation****\n");
    Process *running_process = 0;
    int32_t timer = 0; // Time Quantum을 준수하기 위한 타이머 생성
    for (int32_t current_time = 0; current_time < TIMEMAX; current_time++)
    {
        ...
        if (is_null_process(running_process))
        {
            ...
            if (get_queue_size() == 0)
            {
                ...
                if (is_done(current_time))
                {
                    write_log("RR Scheduling Simulation Exit\n\n");
                    break;
                }
            }
            else
        ...
        else
        {
            ...
            if (++timer == TQ)
            {
                ...
                write_log("%2zu(ms): p%-2zu process interrupted\n", current_time, running_process->pid);
...
void sim_hrn()
{
    write_log("****HRN Scheduling Simulation****\n");
    Process *running_process = 0;
    ...
    for (int32_t current_time = 0; current_time < TIMEMAX; current_time++)
    {
        ...
        if (is_null_process(running_process))
        {
            ...
            if (get_queue_size() == 0)
            {
                ...
                if (is_done(current_time))
                {
                    write_log("HRN Scheduling Simulation Exit\n\n");
                    break;
                }
            }
            else
        ...
        else
        {
            ...
            if (--running_process->remaining_time == 0)
            {
                ...
                if (get_queue_size() != 0)
                {
                    ...
                    /* highest response ratio를 계산하기 위한 임시변수 */
                    long double hrr = -1;

                    /* response ration가 가장 높은 프로세스의 pid를 담기 위한 임시변수 */
                    int32_t next_process_id = 0;

                    for (int32_t i = 0; i < get_queue_size(); i++)
                    {
                        /*
                         ...
                         */
                        long double temp_hrr = (current_time - remaining_process_array[i]->arrival_time + remaining_process_array[i]->burst_time) / (long double)remaining_process_array[i]->burst_time;
                        if (temp_hrr > hrr)
                        {
                    ...
                    write_log("%2zu(ms): p%zu dispatch process\n", current_time, running_process->pid);
                }
                else
                {
                    // 큐에 프로세스가 없는 경우
...
void print_debug_buffer()
{
    if (DEBUG_MODE)
    {
        char *debug_log = get_debug_log();
        if (DEBUG_MODE & 0b100)
        {
            printf("%s", debug_log + get_split_position(0));
        }
        if (DEBUG_MODE & 0b010)
        {
            printf("%s", debug_log + get_split_position(1));
        }
        if (DEBUG_MODE & 0b001)
        {
            printf("%s", debug_log + get_split_position(2));
        }
        free(debug_log);
    }
}
